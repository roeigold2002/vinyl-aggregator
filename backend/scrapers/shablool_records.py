"""Scraper for Shablool Records (shabloolrecords.co.il)"""
import logging
...
class ShabloolRecordsScraper(BaseScraper):
    """Scraper for Shablool Records - WooCommerce structure"""
    ...
    def __init__(self):
        ...
        super().__init__("shablool_records", "https://shabloolrecords.co.il", config)
    
    def scrape(self) -> List[Dict]:
        """Scrape Shablool Records product listings"""
        logger.info(f"🔄 Starting Shablool Records scrape...")
        
        try:
            base_url = f"{self.base_url}/product-category/vinyl"
            products = self._scrape_with_pagination(base_url)
            
            logger.info(f"✅ Shablool Records scrape completed: {len(products)} products")
            self.records_scraped = len(products)
            return products
        
        except Exception as e:
            error_msg = f"Shablool Records scrape failed: {str(e)}"
