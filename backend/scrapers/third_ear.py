...
class ThirdEarScraper(BaseScraper):
    """Scraper for Third-ear.com - WooCommerce structure"""
    ...
    def __init__(self):
        ...
        super().__init__("third_ear", "https://www.third-ear.com", config)
    
    def scrape(self) -> List[Dict]:
        """Scrape Third-ear product listings"""
        logger.info(f"🔄 Starting Third-ear scrape...")
        
        try:
            base_url = f"{self.base_url}/product-category/vinyls"
            products = self._scrape_with_pagination(base_url)
            
            logger.info(f"✅ Third-ear scrape completed: {len(products)} products")
            self.records_scraped = len(products)
            return products
        
        except Exception as e:
            error_msg = f"Third-ear scrape failed: {str(e)}"
