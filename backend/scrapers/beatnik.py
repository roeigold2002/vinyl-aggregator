...
class BeatnikScraper(BaseScraper):
    """Scraper for Beatnik.co.il - WooCommerce structure"""
    ...
    def __init__(self):
        ...
        super().__init__("beatnik", "https://www.beatnik.co.il", config)
    
    def scrape(self) -> List[Dict]:
        """Scrape Beatnik product listings"""
        logger.info(f"🔄 Starting Beatnik scrape...")
        
        try:
            base_url = f"{self.base_url}/online-store"
            products = self._scrape_with_pagination(base_url)
            
            logger.info(f"✅ Beatnik scrape completed: {len(products)} products")
            self.records_scraped = len(products)
            return products
        
        except Exception as e:
            error_msg = f"Beatnik scrape failed: {str(e)}"
