"""Scraper for Vinyl Stock (vinylstock.co.il)"""
import logging
...
class VinylStockScraper(BaseScraper):
    """Scraper for Vinyl Stock - Custom store platform"""
    
    def __init__(self):
        config = {
            "name": "Vinyl Stock",
            "base_url": "https://www.vinylstock.co.il",
            "product_selector": "div.product-item, div.product-card",
            "title_selector": "h3.product-title, h3.product-name, span.product-title",
            "price_selector": "span.product-price, span.price, div.price",
            "image_selector": "img.product-image, img",
            "stock_selector": "span.stock-badge, div.stock-status, span.stock",
        }
        super().__init__("vinyl_stock", "https://www.vinylstock.co.il", config)
    
    def scrape(self) -> List[Dict]:
        """Scrape Vinyl Stock product listings"""
        logger.info(f"🔄 Starting Vinyl Stock scrape...")
        
        try:
            base_url = f"{self.base_url}/store/vinyl"
            products = self._scrape_with_pagination(base_url)
            
            logger.info(f"✅ Vinyl Stock scrape completed: {len(products)} products")
            self.records_scraped = len(products)
            return products
        
        except Exception as e:
            error_msg = f"Vinyl Stock scrape failed: {str(e)}"
            logger.error(error_msg)
            self.errors.append(error_msg)
            return []
    
    def _scrape_with_pagination(self, base_url: str, max_pages: int = 50) -> List[Dict]:
        """Scrape all products with custom pagination"""
        all_products = []
        page = 1
        scraped_urls = set()
    ...
    def _parse_product(self, product_elem) -> Optional[Dict]:
        ...
        try:
            ...
            in_stock = "אזל" not in stock_text and "Out of stock" not in stock_text and "מוגבל" not in stock_text
