"""Scraper for Giora Records (giorarecords.co.il)"""
import logging
...
class GioraRecordsScraper(BaseScraper):
    """Scraper for Giora Records - WooCommerce structure"""
    ...
    def __init__(self):
        ...
        super().__init__("giora_records", "https://www.giorarecords.co.il", config)
    
    def scrape(self) -> List[Dict]:
        """Scrape Giora Records product listings"""
        logger.info(f"🔄 Starting Giora Records scrape...")
        
        try:
            base_url = f"{self.base_url}/product-category/vinyl"
            products = self._scrape_with_pagination(base_url)
            
            logger.info(f"✅ Giora Records scrape completed: {len(products)} products")
            self.records_scraped = len(products)
            return products
        
        except Exception as e:
            error_msg = f"Giora Records scrape failed: {str(e)}"
